...
\begin{task}{3.3 e)}
	...
	Använd $\cos(bt)\theta(t)\longleftrightarrow \dfrac{s}{s^2+b^2}\cond{\sigma > 0}\cond{b\text{ reellt}}$ och \\
	$\sin(bt)\theta(t)\longleftrightarrow \dfrac{b}{s^2+b^2}\cond{\sigma > 0}\cond{b\text{ reellt}}$.
	\begin{align*}
	f(t)=&
	(\cos2t-\sin2t)\theta(t)=
	\cos2t\theta(t)-\sin2t\theta(t)\longleftrightarrow \\ \longleftrightarrow
	&\frac{s}{s^2+2^2}-\frac{2}{s^2+2^2}=
	\frac{s-2}{s^2+4}\cond{\sigma > 0}
	\end{align*}
	\ans $(\cos2t-\sin2t)\theta(t)\longleftrightarrow\dfrac{s-2}{s^2+4}\cond{\sigma > 0}$
\end{task}